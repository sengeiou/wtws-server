...
func GetLoginUserInfo(loginName string, password string) (v *SUser, err error) {
	o := orm.NewOrm()
	v = &SUser{LoginName: loginName, PassWord: password, UserType: 1, Status: 1, IsDelete: 1}
	if err = o.Read(v, "LoginName", "PassWord", "UserType", "Status", "IsDelete"); err != nil {
		return v, err
	}
	return v, nil
}
...
func GetUserList(userInfo *request_entity.UserList) (userListParams []orm.Params, countInfo int, err error) {
	...
	qd.Select("su.*,os.name as station_name,os.station_id,sr.role_name,sr.role_id").
		From("s_user as su").
	...
	stationArrStr := ""
	if len(userInfo.StationIDs) > 0 {
		for i := 0; i < len(userInfo.StationIDs); i++ {
			if i == len(userInfo.StationIDs)-1 {
				stationArrStr = fmt.Sprintf("%s%d", stationArrStr, userInfo.StationIDs[i])
			} else {
				stationArrStr = fmt.Sprintf("%s%d,", stationArrStr, userInfo.StationIDs[i])
			}
		}
		qd.And(fmt.Sprintf("sus.station_id in (%s)", stationArrStr))
	}
	...
	qd.GroupBy("su.user_id").OrderBy("su.insert_time desc")
...
func CheckRegistUserInfo(workNo string, loginName string) (users []SUser, err error) {
	o := orm.NewOrm()
	qd, _ := orm.NewQueryBuilder("mysql")
	qd.Select("*").From("s_user").Where("is_delete = 1").And("(work_no = ? OR login_name = ?)")
	sql := qd.String()
	users = []SUser{}
	if num, err := o.Raw(sql, workNo, loginName).QueryRows(&users); err != nil || num == 0 {
		if err != nil {
			logs.Error("[mysql]  查询workNo和loginName对应的用户信息失败，失败信息：", err.Error())
		}
		return []SUser{}, errors.New("查询workNo和loginName对应的用户信息失败")
	}
