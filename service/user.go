...
func GetUserList(data *request_entity.UserList) common_struct.ResponseStruct {
	...
	if userListParams, count, getUserListErr := wtws_mysql.GetUserList(data); getUserListErr != nil {
		...
		for _, itemParam := range userListParams {
			...
			userStationListArr := []service_struct.UserListItemStation{}
			userStationList := wtws_mysql.GetUserStation(userListItem.UserID)
			for _, itemStation := range userStationList {
				userStationListArr = append(userStationListArr, service_struct.UserListItemStation{
					StationID:   itemStation.Id,
					StationName: itemStation.Name,
				})
			}

			userListItem.Station, _ = jsoniter.MarshalToString(userStationListArr)
...
func AddUser(data *request_entity.AddUserInfo) common_struct.ResponseStruct {
	...
	if existEsers, _ := wtws_mysql.CheckRegistUserInfo(data.WorkNo, data.LoginName); len(existEsers) != 0 {
		return common.ResponseStatus(-6, "当前工号和账号对应的用户已存在", nil)
	}

	var birthDate time.Time
	var birthDateErr error
	if birthDate, birthDateErr = time.Parse("2006-01-02T15:04:05Z", data.BirthDate); birthDateErr != nil {
	...
	addUserStationErrs := []error{}
	for _, stationItem := range data.Stations {
		userStation := wtws_mysql.SUserStation{
			StationId:  stationItem.StationID,
			UserId:     int(addUserID),
			InsertTime: time.Now(),
			UpdateTime: time.Now(),
		}
		if userStationID, addUserStationErr := wtws_mysql.AddSUserStation(&userStation); addUserStationErr != nil || userStationID <= 0 {
			logs.Error("[service]  给用户关联服务站失败，用户ID：", addUserID, "\t服务站ID：", userStation.StationId, "\t失败信息:", addUserStationErr.Error())
			addUserStationErrs = append(addUserStationErrs, errors.New("给用户关联服务站失败"))
		}

	}

	if len(addUserStationErrs) > 0 {
		return common.ResponseStatus(500, "", nil)
	}
